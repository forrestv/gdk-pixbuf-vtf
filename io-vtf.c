...
static uint16_t read_byte(const uint8_t **data) {
    uint8_t val = **data;
    ++ *data;
    return val;
}
...
gdk_pixbuf__vtf_image_stop_load (gpointer context_ptr, GError **error)
{
    ...
    if (context->buffer_data_size < VTF_70_HEADER_SIZE) {
        goto corrupt;
    }
    
    memset(&header, 0, sizeof(header));

    const guchar *ptr = context->buffer;
    memcpy(header.signature, ptr, 4);
    ptr += 4;

    if (memcmp(header.signature, "VTF\0", 4) != 0) {
        goto corrupt;
    }

    // read header
    header.version[0]         = read_le32(&ptr);
    header.version[1]         = read_le32(&ptr);
    header.headerSize         = read_le32(&ptr);
    header.width              = read_le16(&ptr);
    header.height             = read_le16(&ptr);
    header.flags              = read_le32(&ptr);
    header.frames             = read_le16(&ptr);
    header.firstFrame         = read_le16(&ptr);
    ptr += 4; // padding
    header.reflectivity[0]    = read_float(&ptr);
    header.reflectivity[1]    = read_float(&ptr);
    header.reflectivity[2]    = read_float(&ptr);
    ptr += 4; // padding
    header.bumpmapScale       = read_float(&ptr);
    header.highResImageFormat = read_le32(&ptr);
    header.mipmapCount        = read_byte(&ptr);
    header.lowResImageFormat  = read_le32(&ptr);
    header.lowResImageWidth   = read_byte(&ptr);
    header.lowResImageHeight  = read_byte(&ptr);

    if (header.version[0] > 7 || (header.version[0] == 7 && header.version[1] >= 1)) {
        if (context->buffer_data_size < VTF_71_HEADER_SIZE) {
            goto corrupt;
        }
        header.depth          = read_le16(&ptr);
    }

    if (header.version[0] > 7 || (header.version[0] == 7 && header.version[1] >= 3)) {
        if (context->buffer_data_size < VTF_73_HEADER_SIZE) {
            goto corrupt;
        }
        ptr += 3; // padding
        header.resources      = read_le32(&ptr);
    }
    ...
    // TODO: support gdk pixbuf animation if frames > 1
    GdkPixbuf* pixbuf = NULL;
//    fprintf(stderr,"version %d.%d, highResImageFormat: %d\n", header.version[0], header.version[1], header.highResImageFormat);
    
    size_t lowresSize = frame_size(header.lowResImageFormat, header.lowResImageWidth, header.lowResImageHeight);
    size_t frameSize = frame_size(header.highResImageFormat, header.width, header.height);
//    size_t resourceHeaderSize = header.resources * VTF_RESOURCE_HEADER_SIZE;
//    size_t faceCount = face_count(&header);

    // XXX: mipmaps have different sizes!
//    size_t dataSize = header.headerSize + lowresSize + resourceHeaderSize + header.frames * faceCount * header.mipmapCount * frameSize;

    if (lowresSize == SIZE_MAX || frameSize == SIZE_MAX) {
        goto unsupported;
    }

//    if (dataSize > context->buffer_data_size) {
//        goto corrupt;
//    }

    // XXX: this is not the way to do this!
    if (context->buffer_data_size < frameSize) {
        goto corrupt;
    }

    size_t pos = context->buffer_data_size - frameSize;
    size_t i, j;
